...
with ThreadPoolExecutor(max_workers=8) as executor:
    futures = [executor.submit(tmb.ticker_prediction, rows=row, thread_lock=yf_lock,use_ensemble=True) for index, row in watchlist_df.iterrows()]
...
html,text = tpc.build_prediction_email(trade_opportunities, .4)
print(tpc.send_email("tradepredict247 email",
               ["msunarja@hotmail.com"],
               "Stock Swing Trade Opportunities (Machine Learning)",
               html,
               text))